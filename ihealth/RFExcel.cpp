...
//
CString IllusionExcelFile::GetCellString(long irow, long icolumn, bool datetype)
{
...
void IllusionExcelFile::SetCellString(long irow, long icolumn,std::wstring new_string)
{
    COleVariant new_value(new_string.c_str());

	std::wstring coltable= _T("0ABCDEFGHIJKLMNOPQRSTUVWXYZ");
	wchar_t cellname[32] = _T("");
	wsprintf(cellname, _T("%c%d"), coltable[icolumn], irow);
	COleVariant name(cellname); 

	CRange range = excel_current_range_.get_Range(name, covOptional);
	range.put_Value2(new_value);
	range.ReleaseDispatch();
