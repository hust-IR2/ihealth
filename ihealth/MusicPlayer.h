...
typedef struct tagMEDIA
{
	...
	std::wstring  name;              //名称
	std::wstring  pathFileName;      //文件路径名
	std::wstring  logoPath;		   //logo图片的路径
	std::wstring	exName;			   //扩展名
	UINT		playNum;		   //播放次数
	std::wstring	size;			   //大小  (M)
	std::wstring totalTime;         //总时间	
	std::wstring  tag;			   //保留字段，可以存放额外信息
...
public:
	...
	TStreamTime GetMusicLenght(ZPlay *play, std::wstring& path);	 //获取歌曲事件长度
	...
	void OpenMedia(const MEDIA& media, BOOL IsManual = false);    //打开媒体对象，IsManual手动操作打开媒体？
	void Play();
	void Pause();
	void Resume();					  //从暂停恢复
	void Stop();
