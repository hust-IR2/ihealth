...
{
	TStreamInfo pInfo;
	if (m_pZplayer->OpenFileW(m_CurMedia.pathFileName.c_str(), sfAutodetect))
		m_pZplayer->GetStreamInfo(&pInfo);

	return pInfo.Length;
}

TStreamTime CMusicPlayer::GetMusicLenght(ZPlay*play, std::wstring& path)
{
	TStreamInfo pInfo;
	if (play->OpenFileW(path.c_str(), sfAutodetect))
		play->GetStreamInfo(&pInfo);
...
void CMusicPlayer::OpenMedia(const MEDIA& media, BOOL IsManual/* = false*/)
{
	m_CurMedia = media;
...
UINT CMusicPlayer::MessageBox( LPCTSTR lpText, UINT uIcon/* = MB_ICONWARNING */, LPCTSTR lpCaption /*= _T("提示")*/,UINT uType /*= MB_OK */)
{
	...
	return 0;//MessageBox.DuiMessageBox(m_pMainFrame, lpText, lpCaption, uType, uIcon);
}

void CMusicPlayer::Play()
{
	if (m_CurMedia.pathFileName == _T(""))
		return;
	if (GetPlayState() != P_STOP)
	{
		m_pZplayer->Stop();
	...
	if (m_pZplayer->OpenFileW(m_CurMedia.pathFileName.c_str(), sfAutodetect) == 0)
	{
		m_pZplayer->Close();
